    ...
    <div class="container mt-5">
        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                // Calculate days difference for aging buckets
                function calculateDaysDiff(docDate) {
                    try {
                        // Expecting docDate in 'd-m-Y' format (e.g., '01-09-2025')
                        if (!docDate || docDate === 'N/A' || docDate === 'Invalid Date') {
                            console.warn(`Invalid doc_date: ${docDate}`);
                            return -1; // Exclude invalid dates
                        }
                        const [day, month, year] = docDate.split('-').map(Number);
                        const parsedDate = new Date(year, month - 1, day);
                        if (isNaN(parsedDate.getTime())) {
                            console.warn(`Invalid date format: ${docDate}`);
                            return -1;
                        }
                        const currentDate = new Date(); // Use current date
                        const diffTime = currentDate - parsedDate;
                        const diffDays = Math.floor(diffTime / (1000 * 60 * 60 * 24));
                        return diffDays >= 0 ? diffDays : -1; // Exclude future dates
                    } catch (error) {
                        console.error(`Error parsing date ${docDate}:`, error);
                        return -1;
                    }
                }
                ...
                $('.aging-card').on('click', function() {
                    ...
                    $('#reportsTable tbody tr').each(function() {
                        ...
                        const riSumInsText = $row.find('td[data-field="ri_sum_ins"]').text().trim();
                        const riSumIns = riSumInsText === 'N/A' ? 0 : parseFloat(riSumInsText.replace(/,/g, '')) || 0;

                        const daysOld = calculateDaysDiff(docDate);
                        console.log(`Doc Date: ${docDate}, Days Old: ${daysOld}, RI Sum Ins: ${riSumIns}`);
                    ...
                    console.log(`Filtered ${count} records for ${label}, Total RI Sum Insured: ${totalRiSumIns}`);
                    ...
                    $('#filteredRecordsTable').append(`
                        <tfoot>
                            <tr class="table-active">
                                <td colspan="9" class="text-end fw-bold">Total RI Sum Insured:</td>
                                <td class="fw-bold" id="modalTotalRiSumIns">${totalRiSumIns.toLocaleString('en-US')}</td>
                                <td colspan="14"></td>
                            </tr>
                        </tfoot>
                    `);
