            ...
            <div class="col-xl-12">
                <div class="card">
                    <div class="card-header border-0 pb-0">
                        <h4 class="card-title">D/O Claims Summary Report</h4>
                    </div>
                    ...
                    <div class="card-body">
                        ...
                        <form class="form-horizontal" role="form" method="GET" 
                              action="{{ route('do.summary.report') }}" 
                              autocomplete="off" 
                              id="reportForm">
                        ...
                        <div class="row mt-4">
                            <div class="col-md-12">
                                <div class="table-responsive">
                                    <table class="table table-striped table-bordered" id="doSummaryTable">
                                        <thead>
                                        ...
                                        <tbody>
                                            @forelse($summaryData as $index => $do)
                                            <tr>
                                                <td>{{ $index + 1 }}</td>
                                                <td><strong>{{ $do['do_name'] ?? 'Unknown D/O' }}</strong></td>
                                                <td class="text-center">{{ number_format($do['count']) }}</td>
                                                <td class="text-right">{{ number_format($do['iev_sum']) }}</td>
                                                <td class="text-right">{{ number_format($do['premium_sum']) }}</td>
                                                <td class="text-right">{{ number_format($do['totclaims_sum']) }}</td>
                                            </tr>
                                            @empty
                                            <tr>
                                                <td colspan="6" class="text-center text-muted py-4">
                                                    ...
                                                    <p>No D/O data found for the selected period {{ date('d-M-Y', strtotime($fromDate)) }} to {{ date('d-M-Y', strtotime($toDate)) }}</p>
                                                </td>
                                            </tr>
                                            @endforelse
                                        </tbody>
                        ...
                        <div class="row mt-4">
                            <div class="col-md-12">
                                <div class="alert alert-info">
                                    <i class="fa fa-info-circle me-2"></i> 
                                    Please select date range and click "Generate Report" to view D/O summary data.
                                </div>
                            </div>
                        </div>
                        @endif
...
$(document).ready(function() {
    if ($('#doSummaryTable').length && $('#doSummaryTable tbody tr:not(.no-data)').length > 0) {
        var table = $('#doSummaryTable').DataTable({
            "pageLength": 25,
            ...
            "language": {
                "search": "Search D/Os:",
                "lengthMenu": "Show _MENU_ D/Os per page",
                "info": "Showing _START_ to _END_ of _TOTAL_ D/Os",
                "infoEmpty": "No D/Os available",
                "infoFiltered": "(filtered from _MAX_ total D/Os)",
                "paginate": {
                    "first": "First",
                    "last": "Last",
                    "next": "Next",
            ...
            "buttons": [
                {
                    ...
                    title: 'DO_Summary_Report_{{ $fromDate ?? date("Y-m-d") }}_{{ $toDate ?? date("Y-m-d") }}',
                    exportOptions: {
                        columns: ':visible'
                    }
                },
            ...
            "initComplete": function() {
                ...
                }).container().appendTo($('#doSummaryTable_wrapper .col-md-6:eq(0)'));
            }
        });
        ...
        table.on('draw', function() {
            ...
            if (api.rows({ search: 'applied' }).count() !== api.rows().count()) {
                $('#doSummaryTable tfoot').html('<tr class="table-dark">' +
